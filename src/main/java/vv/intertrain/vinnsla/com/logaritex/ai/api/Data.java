...
import vv.intertrain.vinnsla.com.logaritex.ai.api.FileApi;
...
public class Data {
	 ...
	 * @param file_ids A list of {@link File} IDs attached to this assistant. There can be a maximum of 20 files
	 ...
	 *
	 * @param messages A list of {@link Message}s to start the thread with.
	 * @param metadata Set of 16 key-value pairs that can be attached to an object. This can be useful for storing
	 * additional information about the object in a structured format. Keys can be a maximum of 64 characters long and
	 * values can be a maximum of 512 characters long.
	 */
	...
	public record Content(Type type, Text text, ImageFile image_file) {
		...
		public Content(ImageFile imageFile) {
			this(Type.image_file, null, imageFile);
		}
	...
	/**
	 * Represents an execution run on a {@link Thread}.
	 *
	 * @param id The identifier, which can be referenced in API endpoints.
	 ...
	 */
	@JsonInclude(Include.NON_NULL)
	public record RunRequest(String assistant_id, String model, String instructions, List<Tool> tools,
	...
	public record RunThreadRequest(String assistant_id, ThreadRequest thread, String model, String instructions,
                                   List<Tool> tools, Map<String, String> metadata) {
	...
	/**
	 * Represents a step in execution of a {@link Run}. It provides a detailed list of steps the Assistant took as
	 ...
	 * @param assistant_id The ID of the {@link Assistant} associated with the run step.
	 * @param thread_id The ID of the {@link Thread} that was run.
	 * @param run_id The ID of the {@link Run} that this run step is a part of.
